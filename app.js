...
function nextStep(step) {
    if (step === 2 && !bookingState.apptType) { showToast("Please select the type of visit.", "error"); return; }
    if (step === 3 && !bookingState.doctor) { showToast("Please select a doctor.", "error"); return; }
    if (step === 4 && !bookingState.time) { showToast("Please select a time slot.", "error"); return; }
    if (step === 5) {
